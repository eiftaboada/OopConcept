...
public class LevelFiveFightingScene extends JPanel {

    // copy paste rani siyas level 1 
    ...
    public LevelFiveFightingScene(JFrame frame, String selectedCharacter) {
        this.frame             = frame;
        ...
        URL bgURL = getClass().getResource("/images/LevelFiveBg.jpg");
        ...
        updateButtons();
        logDivider();
        logMsg("  BATTLE STARTED  —  " + selectedCharacter.toUpperCase() + " vs QUINVER", LOG_SYSTEM);
        logDivider();
    }
    ...
    private void buildTopPanel() {
        ...
        JLabel levelLabel = new JLabel("LEVEL 5 FINAL ─  QUINVER FINAL BOSS", SwingConstants.CENTER);
    ...
    private void buildArena() {
        ...
        enemy = buildSpriteLabel("Quinver", 190, 190);
        arenaPanel.add(enemy);

        enemyName = new JLabel("QUINVER", SwingConstants.CENTER);
...
//        if (playerChar.getMP() < manaCost) {
//            logMsg("  Not enough MP! Need " + manaCost + ", have " + playerChar.;(), LOG_SYSTEM);
//            return;
//        }
        ...
        if (boss.getBossHP() <= 0) {
            logDivider();
            logMsg("  QUINVER DEFEATED! Heading to the store...", LOG_VICTORY);
            logDivider();
            disableButtons();
        ...
        logDivider();
        logMsg("  QUINVER'S TURN", LOG_ENEMY);

        float dmgToPlayer = boss.attack();
        logMsg("  QUINVER attacks for " + (int) dmgToPlayer + " damage!", LOG_DAMAGE);
    ...
    private JButton createSkillButton(String title, String subtitle, String cost, Color bgTint) {
        // Store text as client properties — no child components that block mouse events
    ...
    // ── skill name helpers (customize per character if needed) ──
    ...
    protected void goToStore() {
        ...
        frame.setContentPane(new Store(frame, introBg, selectedCharacter, 6));
        frame.revalidate();
        frame.repaint();
    }
