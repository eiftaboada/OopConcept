...
public class LevelOneFightingScene extends JPanel {
    ...
    public LevelOneFightingScene(JFrame frame, String selectedCharacter) {
        ...
        // bg
        URL bgURL = getClass().getResource("/images/LevelOneBg.jpg");
        ...
        updateButtons();
        logDivider();
        logMsg("  BATTLE STARTED  —  " + selectedCharacter.toUpperCase() + " vs RAVEN", LOG_SYSTEM);
        logDivider();
    }
    ...
    private void buildArena() {
        ...
        playerChar.setMP(playerChar.getMaxMP()); 
        ...
        // for bosslot
        enemy = buildSpriteLabel("Raven", 190, 190);
...
//            return;
//        }
//     kani ma gamit ni??? mao ni error mo zero mana kalit
        ...
        if (boss.getBossHP() <= 0) {
            logDivider();
            logMsg("  RAVEN DEFEATED! Heading to the store...", LOG_VICTORY);
            logDivider();
            disableButtons();
        ...
        //counter
        logDivider();
        logMsg("  RAVEN'S TURN", LOG_ENEMY);

        float dmgToPlayer = boss.attack();
        logMsg("  RAVEN attacks for " + (int) dmgToPlayer + " damage!", LOG_DAMAGE);
    ...
    //
    //Start of logic sa game ing anii ninyu ika arrange natuss logics
    //
    ...
    private void updateBars() {
        ...
        enemyHP.setValue(bHP);  // ← THIS WILL NOW SHOW 0 ON DEFEAT
        ...
        if (boss.getBossHP() > 0) {
            ...
            enemyMpText.setForeground(LOG_DEFEAT);  // Red "DEFEATED" text
        }
    ...
    // store transition if ma daug ma 0 ang hp sa boss, else mag make pa us ug gameoverpanel na mo show if ma zero ang hp sa character 
    ...
    protected void paintComponent(Graphics g) {
        ...
        // subtle ground line below arena sprites
